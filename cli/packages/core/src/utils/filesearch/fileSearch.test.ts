...
describe('FileSearch', () => {
  ...
  it('should use .blackboxignore rules', async () => {
    ...
    expect(results).toEqual(['src/', '.blackboxignore', 'src/not-ignored.js']);
  });

  it('should combine .gitignore and .blackboxignore rules', async () => {
    tmpDir = await createTmpDir({
      '.gitignore': 'dist/',
      '.blackboxignore': 'build/',
