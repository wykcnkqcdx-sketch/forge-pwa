...
import { DEFAULT_BLACKBOX_FLASH_MODEL } from '../config/models.js';
...
describe('checkNextSpeaker', () => {
  ...
  it('should call generateJson with DEFAULT_BLACKBOX_FLASH_MODEL', async () => {
    (chatInstance.getHistory as Mock).mockReturnValue([
      { role: 'model', parts: [{ text: 'Some model output.' }] },
    ] as Content[]);
    const mockApiResponse: NextSpeakerResponse = {
    ...
    const generateJsonCall = (mockGeminiClient.generateJson as Mock).mock
      .calls[0];
    expect(generateJsonCall[3]).toBe(DEFAULT_BLACKBOX_FLASH_MODEL);
  });
});
