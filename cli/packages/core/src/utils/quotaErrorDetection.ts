...
export function isBlackboxQuotaExceededError(error: unknown): boolean {
  // Check for Blackbox insufficient quota errors (should not retry)
  const checkMessage = (message: string): boolean => {
...
export function isBlackboxThrottlingError(error: unknown): boolean {
  // Check for Blackbox throttling errors (should retry)
  const checkMessage = (message: string): boolean => {
    const lowerMessage = message.toLowerCase();
    return (
