...
export async function clearCachedCredentialFile() {
  try {
    ...
    /**
     * Also clear Blackbox SharedTokenManager cache and credentials file to prevent stale credentials
    ...
    try {
      const { SharedTokenManager } = await import(
        '../blackbox/sharedTokenManager.js'
      );
      const { clearBlackboxCredentials } = await import('../blackbox/blackboxOAuth2.js');
      ...
      console.debug('Could not clear Blackbox credentials:', blackboxError);
    }
  } catch (e) {
