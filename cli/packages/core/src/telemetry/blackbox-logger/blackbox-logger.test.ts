...
import { BlackboxLogger, TEST_ONLY } from './blackbox-logger.js';
...
describe('BlackboxLogger', () => {
  ...
  describe('getInstance', () => {
    it('returns undefined when usage statistics are disabled', () => {
      ...
      const logger = BlackboxLogger.getInstance(config);
      expect(logger).toBeUndefined();
    });

    it('returns an instance when usage statistics are enabled', () => {
      const logger = BlackboxLogger.getInstance(mockConfig);
      expect(logger).toBeInstanceOf(BlackboxLogger);
    });

    it('is a singleton', () => {
      const logger1 = BlackboxLogger.getInstance(mockConfig);
      const logger2 = BlackboxLogger.getInstance(mockConfig);
  ...
  describe('event queue management', () => {
    it('should handle event overflow gracefully', () => {
      ...
      const logger = BlackboxLogger.getInstance(debugConfig)!;
      const consoleSpy = vi
        .spyOn(console, 'debug')
        .mockImplementation(() => {});
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          'BlackboxLogger: Dropped old event to prevent memory leak',
        ),
      );
    });
    ...
    it('should handle enqueue errors gracefully', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        'BlackboxLogger: Failed to enqueue log event.',
        expect.any(Error),
      );
  ...
  describe('failed event retry mechanism', () => {
    it('should requeue failed events with size limits', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('BlackboxLogger: Re-queued'),
      );
    });
    ...
    it('should handle empty retry queue gracefully', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('BlackboxLogger: No events re-queued'),
      );
    });
  });
