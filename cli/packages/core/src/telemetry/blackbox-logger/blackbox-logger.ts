...
export class BlackboxLogger {
  private static instance: BlackboxLogger;
  ...
  static getInstance(config?: Config): BlackboxLogger | undefined {
    if (config === undefined || !config?.getUsageStatisticsEnabled())
      return undefined;
    if (!BlackboxLogger.instance) {
      BlackboxLogger.instance = new BlackboxLogger(config);
      process.on(
        'exit',
        BlackboxLogger.instance.shutdown.bind(BlackboxLogger.instance),
      );
    }

    return BlackboxLogger.instance;
  }
  ...
  enqueueLogEvent(event: RumEvent): void {
    try {
      ...
      if (wasAtCapacity && this.config?.getDebugMode()) {
        console.debug(
          `BlackboxLogger: Dropped old event to prevent memory leak (queue size: ${this.events.size})`,
        );
      }
    } catch (error) {
      if (this.config?.getDebugMode()) {
        console.error('BlackboxLogger: Failed to enqueue log event.', error);
      }
    }
  }
  ...
  async flushToRum(): Promise<LogResponse> {
    if (this.isFlushInProgress) {
      if (this.config?.getDebugMode()) {
        console.debug(
          'BlackboxLogger: Flush already in progress, marking pending flush.',
        );
      }
  ...
  private requeueFailedEvents(eventsToSend: RumEvent[]): void {
    ...
    if (numEventsToRequeue === 0) {
      if (this.config?.getDebugMode()) {
        console.debug(
          `BlackboxLogger: No events re-queued (queue size: ${this.events.size})`,
        );
      }
      return;
    }
    ...
    if (this.config?.getDebugMode()) {
      console.debug(
        `BlackboxLogger: Re-queued ${numEventsToRequeue} events for retry (queue size: ${this.events.size})`,
      );
    }
  }
}
