...
describe('Core System Prompt (prompts.ts)', () => {
  ...
  it('should return the base prompt when no userMemory is provided', () => {
    ...
    expect(prompt).toContain('You are Blackbox Code, an interactive CLI agent'); // Check for core content
  ...
  it('should return the base prompt when userMemory is empty string', () => {
    ...
    const prompt = getCoreSystemPrompt('');
    expect(prompt).not.toContain('---\n\n');
    expect(prompt).toContain('You are Blackbox Code, an interactive CLI agent');
  ...
  it('should append userMemory with separator when provided', () => {
    ...
    expect(prompt).toContain('You are Blackbox Code, an interactive CLI agent'); // Ensure base prompt follows
...
describe('Model-specific tool call formats', () => {
  ...
  it('should use XML format for blackbox3-coder model', () => {
    vi.mocked(isGitRepository).mockReturnValue(false);
    const prompt = getCoreSystemPrompt(undefined, undefined, 'blackbox3-coder-7b');
  ...
  it('should use JSON format for blackbox-vl model', () => {
    vi.mocked(isGitRepository).mockReturnValue(false);
    const prompt = getCoreSystemPrompt(undefined, undefined, 'blackbox-vl-max');
  ...
  it('should preserve model-specific formats with sandbox environment', () => {
    ...
    vi.mocked(isGitRepository).mockReturnValue(false);
    const prompt = getCoreSystemPrompt(undefined, undefined, 'blackbox-vl-plus');
