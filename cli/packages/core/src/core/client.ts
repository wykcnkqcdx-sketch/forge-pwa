...
export class GeminiClient {
  ...
  async generateJson(
    ...
    try {
      ...
      const finalSystemInstruction = config.systemInstruction
        ...
        : getCoreSystemPrompt(userMemory, {}, modelToUse, this.config.getApprovalMode() === ApprovalMode.YOLO);
   ...
   */
  private async handleBlackboxOAuthError(error?: unknown): Promise<string | null> {
    if (!error) {
      return null;
    }
    ...
    if (isAuthError) {
      ...
      console.log(
        'Note: If this persists, you may need to re-authenticate with Blackbox OAuth',
      );
      return null;
    }
