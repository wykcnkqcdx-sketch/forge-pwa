...
export class OpenAIContentConverter {
  ...
  private processContent(
    ...
    if (parsedParts.functionResponses.length > 0) {
      ...
      // If there is image or text accompanying function response (eg. browser use),
      // we need to add it as a separate user message AFTER the tool messages
      // to maintain proper message ordering for Anthropic API
      if (parsedParts.mediaParts.length > 0 || parsedParts.textParts.length > 0) {
