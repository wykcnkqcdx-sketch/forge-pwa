...
export class ContentGenerationPipeline {
  ...
  private addCacheControlToContent(
    ...
    if (Array.isArray(message.content)) {
      let lastTextPart = message.content
      ...
      // @ts-expect-error - Adding cache_control to text part for Anthropic models
      lastTextPart['cache_control'] = { type: 'ephemeral' }
    }
  }

  private async addPromptCacheControl(
    messages: OpenAI.Chat.ChatCompletionMessageParam[],
  ): Promise<OpenAI.Chat.ChatCompletionMessageParam[]> {
    const systemMessage = messages.find((msg) => msg.role === "system")
    if (systemMessage) {
      this.addCacheControlToContent(systemMessage)
    }

    const lastTwoUserMessages = messages.filter((msg) => msg.role === "user").slice(-2)
    lastTwoUserMessages.forEach((msg) => {
      this.addCacheControlToContent(msg)
    })
  ...
  private isAnthropicModel(model: string): boolean {
    return (model.includes("claude") || model.includes("sonnet") || 
            model.includes("opus") || model.includes("anthropic") || 
            model.includes("blackbox-pro"))
  }
  ...
  private async buildRequest(
    ...
    const isAnthropicModel = this.isAnthropicModel(this.contentGeneratorConfig.model);
    if (isAnthropicModel) {
      messages = await this.addPromptCacheControl(messages);
    }
    ...
    const baseRequest: OpenAI.Chat.ChatCompletionCreateParams = {
      ...
      messages,
      // ...(isAnthropicModel ? { usage: {include: true }} : {}),
  ...
  private buildSamplingParameters(
    ...
    // Get the model's maximum output token limit as the default
    const modelOutputLimit = tokenLimit(this.contentGeneratorConfig.model, 'output');
    ...
    const params = {
      ...
      // Use model's output limit as default to enable maximum output capacity
      ...addParameterIfDefined('max_tokens', 'max_tokens', 'maxOutputTokens', modelOutputLimit),
    ...
    if(this.isAnthropicModel(this.contentGeneratorConfig.model)) {
      return params;
    }

    // if not anthropic model, add top_p
    return {
      ...params,
