...
describe('DashScopeOpenAICompatibleProvider', () => {
  ...
  describe('isDashScopeProvider', () => {
    it('should return true for BLACKBOX_OAUTH auth type', () => {
      const config = {
        authType: AuthType.BLACKBOX_OAUTH,
  ...
  describe('buildHeaders', () => {
    it('should build DashScope-specific headers', () => {
      ...
      expect(headers).toEqual({
        'User-Agent': `BlackboxCode/1.0.0 (${process.platform}; ${process.arch})`,
        'X-DashScope-CacheControl': 'enable',
        'X-DashScope-UserAgent': `BlackboxCode/1.0.0 (${process.platform}; ${process.arch})`,
        'X-DashScope-AuthType': AuthType.BLACKBOX_OAUTH,
      });
    });
    ...
    it('should handle unknown CLI version', () => {
      ...
      expect(headers['User-Agent']).toBe(
        `BlackboxCode/unknown (${process.platform}; ${process.arch})`,
      );
  ...
  describe('buildRequest', () => {
    ...
    it('should preserve all original request parameters', () => {
      ...
      expect(result.model).toBe('blackbox-max');
  ...
  describe('output token limits', () => {
    it('should limit max_tokens when it exceeds model limit for blackbox3-coder-plus', () => {
    ...
    it('should limit max_tokens when it exceeds model limit for blackbox-vl-max-latest', () => {
      const request: OpenAI.Chat.ChatCompletionCreateParams = {
        model: 'blackbox-vl-max-latest',
        messages: [{ role: 'user', content: 'Hello' }],
    ...
    it('should set high resolution flag for blackbox3-vl-plus', () => {
      const request: OpenAI.Chat.ChatCompletionCreateParams = {
        model: 'blackbox3-vl-plus',
        messages: [
          {
            role: 'user',
            content: [
