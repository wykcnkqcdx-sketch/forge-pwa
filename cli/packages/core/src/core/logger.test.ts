...
const GEMINI_DIR_NAME = '.blackboxcli';
...
describe('Logger', () => {
  ...
  describe('Model Switch Logging', () => {
    it('should log model switch events correctly', async () => {
      ...
      expect(loggedEvent.fromModel).toBe('blackbox3-coder-plus');
      expect(loggedEvent.toModel).toBe('blackbox-vl-max-latest');
    ...
    it('should handle multiple model switch events', async () => {
      ...
      await logger.logModelSwitch({
        fromModel: 'blackbox-vl-max-latest',
        toModel: 'blackbox3-coder-plus',
        reason: 'vision_auto_switch',
      ...
      expect(firstSwitch.fromModel).toBe('blackbox3-coder-plus');
      expect(firstSwitch.toModel).toBe('blackbox-vl-max-latest');

      const secondSwitch = JSON.parse(modelSwitchLogs[1].message);
      expect(secondSwitch.fromModel).toBe('blackbox-vl-max-latest');
      expect(secondSwitch.toModel).toBe('blackbox3-coder-plus');
    });
  });
});
