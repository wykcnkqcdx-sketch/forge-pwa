...
describe('normalize', () => {
  ...
  it('should handle pipe and colon separators', () => {
    expect(normalize('blackbox|blackbox2.5:blackbox2.5-1m')).toBe('blackbox2.5-1m');
  });
  ...
  it('should remove quantization and numeric suffixes', () => {
    expect(normalize('blackbox3-coder-7b-4bit')).toBe('blackbox3-coder-7b');
    ...
    expect(normalize('blackbox2.5-quantized')).toBe('blackbox2.5');
  });
...
describe('tokenLimit', () => {
  ...
  describe('Anthropic Claude', () => {
    ...
    it('should return the correct limit for Claude Sonnet 4.5 (blackbox-pro)', () => {
      expect(tokenLimit('claude-sonnet-4.5')).toBe(1048576);
      expect(tokenLimit('claude-sonnet-4-5-20250929')).toBe(1048576);
      expect(tokenLimit('blackbox-pro')).toBe(1048576);
    });
  ...
  describe('Alibaba Blackbox', () => {
    it('should return the correct limit for blackbox3-coder commercial models', () => {
      expect(tokenLimit('blackbox3-coder-plus')).toBe(1048576);
      expect(tokenLimit('blackbox3-coder-plus-20250601')).toBe(1048576);
      expect(tokenLimit('blackbox3-coder-flash')).toBe(1048576);
      expect(tokenLimit('blackbox3-coder-flash-20250601')).toBe(1048576);
    });

    it('should return the correct limit for blackbox3-coder open source models', () => {
      expect(tokenLimit('blackbox3-coder-7b')).toBe(262144);
      expect(tokenLimit('blackbox3-coder-480b-a35b-instruct')).toBe(262144);
      expect(tokenLimit('blackbox3-coder-30b-a3b-instruct')).toBe(262144);
    });

    it('should return the correct limit for blackbox3 2507 variants', () => {
      expect(tokenLimit('blackbox3-some-model-2507-instruct')).toBe(262144);
    });

    it('should return the correct limit for blackbox2.5-1m', () => {
      expect(tokenLimit('blackbox2.5-1m')).toBe(1048576);
      expect(tokenLimit('blackbox2.5-1m-instruct')).toBe(1048576);
    });

    it('should return the correct limit for blackbox2.5', () => {
      expect(tokenLimit('blackbox2.5')).toBe(131072);
      expect(tokenLimit('blackbox2.5-instruct')).toBe(131072);
    });

    it('should return the correct limit for blackbox-plus', () => {
      expect(tokenLimit('blackbox-plus-latest')).toBe(1048576);
      expect(tokenLimit('blackbox-plus')).toBe(131072);
    });

    it('should return the correct limit for blackbox-flash', () => {
      expect(tokenLimit('blackbox-flash-latest')).toBe(1048576);
    });

    it('should return the correct limit for blackbox-turbo', () => {
      expect(tokenLimit('blackbox-turbo')).toBe(131072);
      expect(tokenLimit('blackbox-turbo-latest')).toBe(131072);
    });
  });
...
describe('tokenLimit with output type', () => {
  describe('Blackbox models with output limits', () => {
    it('should return the correct output limit for blackbox3-coder-plus', () => {
      expect(tokenLimit('blackbox3-coder-plus', 'output')).toBe(65536);
      expect(tokenLimit('blackbox3-coder-plus-20250601', 'output')).toBe(65536);
    });

    it('should return the correct output limit for blackbox-vl-max-latest', () => {
      expect(tokenLimit('blackbox-vl-max-latest', 'output')).toBe(8192);
    });
  });

  describe('Claude models with output limits', () => {
    it('should return the correct output limit for Claude Sonnet 4.5 (blackbox-pro)', () => {
      expect(tokenLimit('claude-sonnet-4.5', 'output')).toBe(64000); // 64K output (Anthropic exact)
      expect(tokenLimit('claude-sonnet-4-5-20250929', 'output')).toBe(64000); // 64K output
      expect(tokenLimit('blackbox-pro', 'output')).toBe(64000); // 64K output
    });

    it('should return the correct output limit for Claude Sonnet 4', () => {
      expect(tokenLimit('claude-sonnet-4', 'output')).toBe(64000); // 64K output (Anthropic exact)
      expect(tokenLimit('claude-sonnet-4-20250514', 'output')).toBe(64000); // 64K output
    });

    it('should return the correct output limit for Claude Opus 4', () => {
      expect(tokenLimit('claude-opus-4', 'output')).toBe(32000); // 32K output (Anthropic exact)
      expect(tokenLimit('claude-opus-4-20250514', 'output')).toBe(32000); // 32K output
    });

    it('should return the correct output limit for Claude 3.7 Sonnet', () => {
      expect(tokenLimit('claude-3.7-sonnet', 'output')).toBe(64000); // 64K output (Anthropic exact)
    });

    it('should return the correct output limit for Claude 3.5 Sonnet', () => {
      expect(tokenLimit('claude-3.5-sonnet', 'output')).toBe(8192); // 8K output
    });
  });

  describe('OpenAI models with output limits', () => {
    it('should return the correct output limit for GPT-4.1 models', () => {
      expect(tokenLimit('gpt-4.1', 'output')).toBe(16384); // 16K output
      expect(tokenLimit('gpt-4.1-mini', 'output')).toBe(16384); // 16K output
    });

    it('should return the correct output limit for GPT-4o models', () => {
      expect(tokenLimit('gpt-4o', 'output')).toBe(16384); // 16K output
      expect(tokenLimit('gpt-4o-mini', 'output')).toBe(16384); // 16K output
    });
  });
  ...
  describe('Default output limits', () => {
    ...
    it('should return the default output limit for models without specific output patterns', () => {
      expect(tokenLimit('blackbox3-coder-7b', 'output')).toBe(
        DEFAULT_OUTPUT_TOKEN_LIMIT,
      );
      expect(tokenLimit('blackbox-plus', 'output')).toBe(
        DEFAULT_OUTPUT_TOKEN_LIMIT,
      );
      expect(tokenLimit('blackbox-vl-max', 'output')).toBe(
        DEFAULT_OUTPUT_TOKEN_LIMIT,
      );
    });
  });
  ...
  describe('Input vs Output limits comparison', () => {
    it('should return different limits for input vs output for blackbox3-coder-plus', () => {
      expect(tokenLimit('blackbox3-coder-plus', 'input')).toBe(1048576); // 1M input
      expect(tokenLimit('blackbox3-coder-plus', 'output')).toBe(65536); // 64K output
    });

    it('should return different limits for input vs output for blackbox-vl-max-latest', () => {
      expect(tokenLimit('blackbox-vl-max-latest', 'input')).toBe(131072); // 128K input
      expect(tokenLimit('blackbox-vl-max-latest', 'output')).toBe(8192); // 8K output
    });

    it('should return different limits for input vs output for blackbox3-vl-plus', () => {
      expect(tokenLimit('blackbox3-vl-plus', 'input')).toBe(262144); // 256K input
      expect(tokenLimit('blackbox3-vl-plus', 'output')).toBe(32768); // 32K output
    });
  ...
  describe('Backward compatibility', () => {
    it('should default to input type when no type is specified', () => {
      expect(tokenLimit('blackbox3-coder-plus')).toBe(1048576); // Should be input limit
      expect(tokenLimit('blackbox-vl-max-latest')).toBe(131072); // Should be input limit
      expect(tokenLimit('unknown-model')).toBe(DEFAULT_TOKEN_LIMIT); // Should be input default
    });

    it('should work with explicit input type', () => {
      expect(tokenLimit('blackbox3-coder-plus', 'input')).toBe(1048576);
      expect(tokenLimit('blackbox-vl-max-latest', 'input')).toBe(131072);
  ...
  describe('Model normalization with output limits', () => {
    ...
    it('should handle complex model strings for output limits', () => {
      expect(
        tokenLimit(
          '  a/b/c|BLACKBOX3-CODER-PLUS:blackbox3-coder-plus-2024-05-13  ',
          'output',
        ),
      ).toBe(65536);
      expect(
        tokenLimit(
          'provider/blackbox-vl-max-latest:blackbox-vl-max-latest-v1',
          'output',
        ),
      ).toBe(8192);
    });
