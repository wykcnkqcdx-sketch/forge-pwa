...
export class Logger {
  private blackboxCliDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.blackboxCliDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.blackboxCliDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.blackboxCliDir, { recursive: true });
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.blackboxCliDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.blackboxCliDir) {
      console.error(
  ...
  async getCheckpointMetadata(tag: string): Promise<{
    timestamp: Date;
    messageCount: number;
    ...
    try {
      const conversation = await this.loadCheckpoint(tag);
      if (conversation.length === 0) {
        return null;
      }

      // Get file stats for timestamp
      const filePath = await this._getCheckpointPath(tag);
      const stats = await fs.stat(filePath);

      // Count messages (excluding system prompt if present)
      let messageCount = conversation.length;
      let startIndex = 0;

      // Check if first message is system prompt
      if (
        conversation.length > 0 &&
        conversation[0].parts?.[0]?.text?.includes('context for our chat')
      ) {
        messageCount--;
        startIndex = 1;
      }
      ...
      console.error(`Failed to get metadata for checkpoint ${tag}:`, error);
      return null;
    }
  }

  async listAllCheckpoints(): Promise<
    Array<{
      tag: string;
      timestamp: Date;
      messageCount: number;
  ...
  > {
    ...
    try {
      const files = await fs.readdir(this.blackboxCliDir);
      ...
      for (const file of files) {
        if (file.startsWith('checkpoint-') && file.endsWith('.json')) {
          const encodedTag = file.slice(
            'checkpoint-'.length,
            -'.json'.length,
          );
          const tag = decodeTagName(encodedTag);

          const metadata = await this.getCheckpointMetadata(tag);
          if (metadata) {
            checkpoints.push({
              tag,
              ...
              isAutoSave: tag.startsWith('autosave-'),
            });
          }
        }
      }
      ...
      console.error('Failed to list all checkpoints:', error);
      return [];
    }
  }
