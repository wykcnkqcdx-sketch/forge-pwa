...
import { ReadDataFileTool } from '../tools/read-data-file.js';
...
export class Config {
  ...
  private readonly saveCheckpoint: string | undefined;
  private readonly autoSave: boolean;
  private readonly resumeCheckpoint: string | undefined;
  private readonly listCheckpoints: boolean;
  private initialized: boolean = false;
  readonly storage: Storage;
  private readonly fileExclusions: FileExclusions;
  private logger: Logger | null = null;
  private supressToolErrors: boolean = false;
  ...
  constructor(params: ConfigParameters) {
    ...
    this.gitCoAuthor = {
      ...
      name: params.gitCoAuthor?.name ?? 'BlackboxAI',
      email: params.gitCoAuthor?.email ?? 'code@blackbox.ai',
    };
    ...
    this.saveCheckpoint = params.saveCheckpoint;
    this.autoSave = params.autoSave ?? false;
    this.resumeCheckpoint = params.resumeCheckpoint;
    this.listCheckpoints = params.listCheckpoints ?? false;
    this.supressToolErrors = params.supressToolErrors ?? false;
   ...
   */
  getMcpServerErrors() {
    return this.toolRegistry?.getMcpServerErrors() ?? [];
  }

  /**
   * Checks if there are any MCP server errors
   */
  hasMcpServerErrors(): boolean {
    return this.toolRegistry?.hasMcpServerErrors() ?? false;
  }
  ...
  getSaveCheckpoint(): string | undefined {
  ...
  getResumeCheckpoint(): string | undefined {
    return this.resumeCheckpoint;
  }

  getListCheckpoints(): boolean {
    return this.listCheckpoints;
  }

  getSupressToolIterationErrors(): boolean {
    return this.supressToolErrors;
  }
  ...
  async createToolRegistry(): Promise<ToolRegistry> {
    ...
    registerCoreTool(ReadDataFileTool, this);
    ...
    // Browser tools
    // registerCoreTool(BrowserLaunchTool, this);
    registerCoreTool(BrowserNavigateTool, this);
    registerCoreTool(BrowserClickTool, this);
    registerCoreTool(BrowserTypeTool, this);
    registerCoreTool(BrowserScrollDownTool, this);
    registerCoreTool(BrowserScrollUpTool, this);
    registerCoreTool(BrowserCloseTool, this);
    registerCoreTool(TodoWriteTool, this);
    registerCoreTool(ExitPlanModeTool, this);
    registerCoreTool(WebFetchTool, this);
    
    // Register web search tool if:
    // 1. Tavily API key is set, OR
    // 2. Using BLACKBOX AI provider or OpenAI/custom provider (which has native web search)
    // Note: We check authType from constructor params since contentGeneratorConfig
    // is not initialized yet when createToolRegistry() is called
    const isBlackboxAI = this.authType === AuthType.USE_BLACKBOX_API;
    
    if (this.getTavilyApiKey() || isBlackboxAI) {
