...
import {
  ...
  isErrorResponse,
  BlackboxOAuth2Client,
  type DeviceAuthorizationResponse,
  type DeviceTokenResponse,
  type ErrorData,
  type BlackboxCredentials,
} from './blackboxOAuth2.js';
import {
  SharedTokenManager,
  TokenManagerError,
  TokenError,
...
interface MockSharedTokenManager {
  getValidCredentials(blackboxClient: BlackboxOAuth2Client): Promise<BlackboxCredentials>;
...
vi.mock('./sharedTokenManager.js', () => ({
  SharedTokenManager: class {
    ...
    async getValidCredentials(
      blackboxClient: BlackboxOAuth2Client,
    ): Promise<BlackboxCredentials> {
      // Try to get credentials from the client first
      const clientCredentials = blackboxClient.getCredentials();
  ...
  describe('isDeviceAuthorizationSuccess', () => {
    it('should return true for successful authorization response', () => {
      const expectedBaseUrl = process.env['DEBUG']
        ? 'https://pre4-chat.blackboxcli.ai'
        : 'https://chat.blackboxcli.ai';
  ...
  describe('isErrorResponse', () => {
    ...
    it('should return false for successful responses', () => {
      const successResponse: DeviceAuthorizationResponse = {
        device_code: 'test-device-code',
        user_code: 'TEST123',
        verification_uri: 'https://chat.blackboxcli.ai/device',
        verification_uri_complete: 'https://chat.blackboxcli.ai/device?code=TEST123',
        expires_in: 1800,
      };
...
describe('BlackboxOAuth2Client', () => {
  let client: BlackboxOAuth2Client;
  let originalFetch: typeof global.fetch;

  beforeEach(() => {
    // Create client instance
    client = new BlackboxOAuth2Client();
    ...
    it('should refresh token if access token is expired', async () => {
      ...
      (
        client as unknown as {
          sharedManager: {
            getValidCredentials: () => Promise<BlackboxCredentials>;
          };
        }
      ).sharedManager = {
...
describe('getBlackboxOAuthClient', () => {
  ...
  it('should load cached credentials if available', async () => {
    ...
    const client = await import('./blackboxOAuth2.js').then((module) =>
      module.getBlackboxOAuthClient(mockConfig),
    );
  ...
  it('should handle cached credentials refresh failure', async () => {
    ...
    const mockCredentials = {
      ...
      token_type: 'Bearer',
      expiry_date: Date.now() + 3600000, // Valid expiry time so loadCachedBlackboxCredentials returns true
    };
    ...
    await expect(
      import('./blackboxOAuth2.js').then((module) =>
        module.getBlackboxOAuthClient(mockConfig),
      ),
    ).rejects.toThrow('Blackbox OAuth authentication failed');
...
describe('clearBlackboxCredentials', () => {
  it('should successfully clear credentials file', async () => {
    const { promises: fs } = await import('node:fs');
    const { clearBlackboxCredentials } = await import('./blackboxOAuth2.js');

    vi.mocked(fs.unlink).mockResolvedValue(undefined);

    await expect(clearBlackboxCredentials()).resolves.not.toThrow();
...
describe('BlackboxOAuth2Client - Additional Error Scenarios', () => {
  let client: BlackboxOAuth2Client;
  let originalFetch: typeof global.fetch;
...
describe('getBlackboxOAuthClient - Enhanced Error Scenarios', () => {
  let mockConfig: Config;
  let originalFetch: typeof global.fetch;
  ...
  it('should handle different authentication failure reasons - timeout', async () => {
    ...
    ).rejects.toThrow('Blackbox OAuth authentication timed out');
  ...
  it('should handle authentication failure reason - rate limit', async () => {
    ...
    await expect(
      ...
      'Too many request for Blackbox OAuth authentication, please try again later.',
    );
...
describe('authWithBlackboxDeviceFlow - Comprehensive Testing', () => {
  let mockConfig: Config;
  let originalFetch: typeof global.fetch;
...
describe('Event Emitter Integration', () => {
  it('should export blackboxOAuth2Events as EventEmitter', async () => {
    const { blackboxOAuth2Events } = await import('./blackboxOAuth2.js');
    expect(blackboxOAuth2Events).toBeInstanceOf(EventEmitter);
  });

  it('should define correct event enum values', async () => {
    const { BlackboxOAuth2Event } = await import('./blackboxOAuth2.js');
    expect(BlackboxOAuth2Event.AuthUri).toBe('auth-uri');
    expect(BlackboxOAuth2Event.AuthProgress).toBe('auth-progress');
    expect(BlackboxOAuth2Event.AuthCancel).toBe('auth-cancel');
  });
});
...
describe('Utility Functions', () => {
  ...
  describe('getBlackboxCachedCredentialPath', () => {
    it('should return correct path to cached credentials', async () => {
      const os = await import('os');
      const path = await import('path');

      const expectedPath = path.join(os.homedir(), '.blackboxcli', 'oauth_creds.json');

      // Since this is a private function, we test it indirectly through clearBlackboxCredentials
      const { promises: fs } = await import('node:fs');
      const { clearBlackboxCredentials } = await import('./blackboxOAuth2.js');

      vi.mocked(fs.unlink).mockResolvedValue(undefined);

      await clearBlackboxCredentials();
...
describe('Credential Caching Functions', () => {
  describe('cacheBlackboxCredentials', () => {
    it('should create directory and write credentials to file', async () => {
      // Mock the internal cacheBlackboxCredentials function by creating client and calling refresh
      const client = new BlackboxOAuth2Client();
      client.setCredentials({
        refresh_token: 'test-refresh',
      });
  ...
  describe('loadCachedBlackboxCredentials', () => {
    it('should load and validate cached credentials successfully', async () => {
      ...
      // Test through getBlackboxOAuthClient which calls loadCachedBlackboxCredentials
      const mockConfig = {
      ...
      try {
        await import('./blackboxOAuth2.js').then((module) =>
          module.getBlackboxOAuthClient(mockConfig),
        );
      } catch {
...
describe('Enhanced Error Handling and Edge Cases', () => {
  ...
  describe('BlackboxOAuth2Client getAccessToken enhanced scenarios', () => {
  ...
  describe('Enhanced refreshAccessToken scenarios', () => {
    it('should call clearBlackboxCredentials on 400 error', async () => {
      client.setCredentials({
        refresh_token: 'expired-refresh',
      });
...
describe('SharedTokenManager Integration in BlackboxOAuth2Client', () => {
  let client: BlackboxOAuth2Client;
  ...
  it('should handle TokenManagerError types correctly in getBlackboxOAuthClient', async () => {
    const mockConfig = {
      isBrowserLaunchSuppressed: vi.fn().mockReturnValue(true),
    } as unknown as Config;
...
describe('Constants and Configuration', () => {
  it('should have correct OAuth endpoints', async () => {
    ...
    expect(url).toBe('https://chat.blackboxcli.ai/api/v1/oauth2/device/code');
  });
