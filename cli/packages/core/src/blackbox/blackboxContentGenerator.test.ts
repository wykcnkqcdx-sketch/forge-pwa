...
import type { IBlackboxOAuth2Client } from './blackboxOAuth2.js';
import { type BlackboxCredentials, type ErrorData } from './blackboxOAuth2.js';
import type {
...
import { BlackboxContentGenerator } from './blackboxContentGenerator.js';
...
vi.mock('./sharedTokenManager.js', () => ({
  SharedTokenManager: class {
    ...
    private mockCredentials: BlackboxCredentials | null = null;
    ...
    async getValidCredentials(
      blackboxClient: IBlackboxOAuth2Client,
    ): Promise<BlackboxCredentials> {
      ...
      try {
        const { token } = await blackboxClient.getAccessToken();
        if (token) {
          const credentials = blackboxClient.getCredentials();
          return credentials;
        }
      } catch (error) {
        ...
        if (isAuthError) {
          ...
          try {
            const refreshResult = await blackboxClient.refreshAccessToken();
            if (refreshResult && !('error' in refreshResult)) {
              // Refresh succeeded, update client credentials and return them
              const updatedCredentials = blackboxClient.getCredentials();
              return updatedCredentials;
            } else {
              // Refresh failed, throw appropriate error
              throw new Error(
                'Failed to obtain valid Blackbox access token. Please re-authenticate.',
              );
            }
          } catch {
            throw new Error(
    ...
    getCurrentCredentials(): BlackboxCredentials | null {
      return this.mockCredentials;
    }
    ...
    setMockCredentials(credentials: BlackboxCredentials | null): void {
...
describe('BlackboxContentGenerator', () => {
  let mockBlackboxClient: IBlackboxOAuth2Client;
  let blackboxContentGenerator: BlackboxContentGenerator;
  let mockConfig: Config;

  const mockCredentials: BlackboxCredentials = {
  ...
  beforeEach(() => {
    ...
    // Create BlackboxContentGenerator instance
    const contentGeneratorConfig = {
      model: 'blackbox-turbo',
      apiKey: 'test-api-key',
      authType: AuthType.BLACKBOX_OAUTH,
      baseUrl: 'https://dashscope.aliyuncs.com/compatible-mode/v1',
      timeout: 120000,
      maxRetries: 3,
    };
    blackboxContentGenerator = new BlackboxContentGenerator(
      mockBlackboxClient,
  ...
  describe('Core Content Generation Methods', () => {
    it('should generate content with valid token', async () => {
      vi.mocked(mockBlackboxClient.getAccessToken).mockResolvedValue({
        token: 'valid-token',
      });
      vi.mocked(mockBlackboxClient.getCredentials).mockReturnValue(mockCredentials);
      ...
      const result = await blackboxContentGenerator.generateContent(
        request,
        'test-prompt-id',
      );

      expect(result.text).toBe('Generated content');
      expect(mockBlackboxClient.getAccessToken).toHaveBeenCalled();
    });
    ...
    it('should generate content stream with valid token', async () => {
      ...
      const stream = await blackboxContentGenerator.generateContentStream(
        request,
        'test-prompt-id',
      );
      const chunks: string[] = [];
    ...
    it('should count tokens without requiring authentication', async () => {
      ...
      const result = await blackboxContentGenerator.countTokens(request);

      expect(result.totalTokens).toBe(15);
      // countTokens is a local operation and should not require OAuth credentials
      expect(mockBlackboxClient.getAccessToken).not.toHaveBeenCalled();
    });
    ...
    it('should embed content with valid token', async () => {
      ...
      const result = await blackboxContentGenerator.embedContent(request);
  ...
  describe('Token Management and Refresh Logic', () => {
    it('should refresh token on auth error and retry', async () => {
      ...
      vi.mocked(mockBlackboxClient.getAccessToken)
        .mockRejectedValueOnce(authError)
        .mockResolvedValueOnce({ token: 'refreshed-token' });

      // Refresh succeeds
      vi.mocked(mockBlackboxClient.refreshAccessToken).mockResolvedValue({
      ...
      vi.mocked(mockBlackboxClient.getCredentials).mockReturnValue({
        access_token: 'refreshed-token',
        token_type: 'Bearer',
      ...
      expect(result.text).toBe('Generated content');
      expect(mockBlackboxClient.refreshAccessToken).toHaveBeenCalled();
    });
    ...
    it('should handle token refresh failure', async () => {
      ...
      await expect(
        blackboxContentGenerator.generateContent(request, 'test-prompt-id'),
      ).rejects.toThrow(
        'Failed to obtain valid Blackbox access token. Please re-authenticate.',
      );
    ...
    it('should update endpoint when token is refreshed', async () => {
      ...
      await blackboxContentGenerator.generateContent(request, 'test-prompt-id');

      expect(mockBlackboxClient.getCredentials).toHaveBeenCalled();
    });
  });
  ...
  describe('Endpoint URL Normalization', () => {
    it('should use default endpoint when no custom endpoint provided', async () => {
      ...
      const parentPrototype = Object.getPrototypeOf(
        Object.getPrototypeOf(blackboxContentGenerator),
      );
  ...
  describe('Client State Management', () => {
    it('should set dynamic credentials during operations', async () => {
      const client = (
        blackboxContentGenerator as unknown as {
  ...
  describe('Error Handling and Retry Logic', () => {
    it('should retry once on authentication errors', async () => {
      ...
      vi.mocked(mockBlackboxClient.getAccessToken).mockImplementation(async () => {
    ...
    it('should not retry non-authentication errors', async () => {
      ...
      expect(mockBlackboxClient.refreshAccessToken).not.toHaveBeenCalled();
    ...
    it('should handle error response from token refresh', async () => {
      vi.mocked(mockBlackboxClient.getAccessToken).mockRejectedValue(
      ...
      await expect(
        blackboxContentGenerator.generateContent(request, 'test-prompt-id'),
      ).rejects.toThrow('Failed to obtain valid Blackbox access token');
    });
  });

  describe('Token State Management', () => {
    it('should cache and return current token', () => {
      expect(blackboxContentGenerator.getCurrentToken()).toBeNull();

      // Simulate setting a token internally
      (
        blackboxContentGenerator as unknown as { currentToken: string }
      ).currentToken = 'cached-token';

      expect(blackboxContentGenerator.getCurrentToken()).toBe('cached-token');
    });

    it('should clear token on clearToken()', () => {
      // Simulate having cached token value
      const blackboxInstance = blackboxContentGenerator as unknown as {
        currentToken: string;
      };
      blackboxInstance.currentToken = 'cached-token';

      blackboxContentGenerator.clearToken();
    ...
    it('should handle concurrent token refresh requests', async () => {
      ...
      vi.mocked(mockBlackboxClient.getAccessToken).mockRejectedValue(authError);
      vi.mocked(mockBlackboxClient.getCredentials).mockReturnValue(mockCredentials);

      vi.mocked(mockBlackboxClient.refreshAccessToken).mockImplementation(
        async () => {
          refreshCallCount++;
  ...
  describe('SharedTokenManager Integration', () => {
    it('should use SharedTokenManager to get valid credentials', async () => {
      ...
      const newGenerator = new BlackboxContentGenerator(
        mockBlackboxClient,
        { model: 'blackbox-turbo', authType: AuthType.BLACKBOX_OAUTH },
        mockConfig,
      );
  ...
  describe('getCurrentEndpoint Method', () => {
    it('should handle URLs with custom ports', () => {
      ...
      endpoints.forEach(({ input, expected }) => {
        ...
        const generator = blackboxContentGenerator as unknown as {
  ...
  describe('Stream Error Handling', () => {
    it('should set credentials when stream generation fails', async () => {
      ...
      try {
        await blackboxContentGenerator.generateContentStream(
          request,
          'test-prompt-id',
        );
      } catch (error) {
  ...
  describe('Constructor and Initialization', () => {
    it('should initialize with configured base URL when provided', () => {
      const generator = new BlackboxContentGenerator(
        mockBlackboxClient,
        {
          model: 'blackbox-turbo',
          authType: AuthType.BLACKBOX_OAUTH,
  ...
  describe('Edge Cases and Error Conditions', () => {
    ...
    it('should handle method types with token failure (except countTokens)', async () => {
      ...
      await expect(newGenerator.embedContent(embedRequest)).rejects.toThrow(
        'Failed to obtain valid Blackbox access token',
      );
