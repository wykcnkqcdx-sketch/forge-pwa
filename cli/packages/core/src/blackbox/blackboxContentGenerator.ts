...
const DEFAULT_BLACKBOX_BASE_URL =
  'https://dashscope.aliyuncs.com/compatible-mode/v1';

/**
 * Blackbox Content Generator that uses Blackbox OAuth tokens with automatic refresh
 */
export class BlackboxContentGenerator extends OpenAIContentGenerator {
  private blackboxCliClient: IBlackboxOAuth2Client;
  private sharedManager: SharedTokenManager;
  private currentToken?: string;

  constructor(
    blackboxCliClient: IBlackboxOAuth2Client,
    contentGeneratorConfig: ContentGeneratorConfig,
    cliConfig: Config,
  ) {
    // Create DashScope provider for Blackbox
    ...
    this.blackboxCliClient = blackboxCliClient;
  ...
  private getCurrentEndpoint(resourceUrl?: string): string {
    const baseEndpoint = resourceUrl || DEFAULT_BLACKBOX_BASE_URL;
    const suffix = '/v1';
  ...
  private async executeWithCredentialManagement<T>(
    ...
    try {
      ...
      if (this.isAuthError(error)) {
        ...
        await this.sharedManager.getValidCredentials(this.blackboxCliClient, true);
        return await attemptOperation();
      }
      throw error;
    }
