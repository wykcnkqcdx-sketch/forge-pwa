...
// OAuth Endpoints
const BLACKBOX_OAUTH_BASE_URL = 'https://chat.blackboxcli.ai';

const BLACKBOX_OAUTH_DEVICE_CODE_ENDPOINT = `${BLACKBOX_OAUTH_BASE_URL}/api/v1/oauth2/device/code`;
const BLACKBOX_OAUTH_TOKEN_ENDPOINT = `${BLACKBOX_OAUTH_BASE_URL}/api/v1/oauth2/token`;

// OAuth Client Configuration
const BLACKBOX_OAUTH_CLIENT_ID = 'f0304373b74a44d2b584a3fb70ca9e56';
...
/**
 * Blackbox OAuth2 credentials interface
 */
export interface BlackboxCredentials {
  access_token?: string;
...
/**
 * Blackbox OAuth2 client interface
 */
export interface IBlackboxOAuth2Client {
...
/**
 * Blackbox OAuth2 client implementation
 */
export class BlackboxOAuth2Client implements IBlackboxOAuth2Client {
  ...
  async requestDeviceAuthorization(options: {
    ...
    const bodyData = {
      client_id: BLACKBOX_OAUTH_CLIENT_ID,
      scope: options.scope,
      code_challenge: options.code_challenge,
      code_challenge_method: options.code_challenge_method,
    };

    const response = await fetch(BLACKBOX_OAUTH_DEVICE_CODE_ENDPOINT, {
      method: 'POST',
      headers: {
  ...
  async pollDeviceToken(options: {
    ...
    const response = await fetch(BLACKBOX_OAUTH_TOKEN_ENDPOINT, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
        Accept: 'application/json',
...
export enum BlackboxOAuth2Event {
  AuthUri = 'auth-uri',
...
/**
 * Global event emitter instance for BlackboxOAuth2 authentication events
 */
export const blackboxOAuth2Events = new EventEmitter();

export async function getBlackboxOAuthClient(
  config: Config,
): Promise<BlackboxOAuth2Client> {
  ...
  try {
    ...
    if (await loadCachedBlackboxCredentials(client)) {
      // We have cached credentials but they might be expired
      // Try device flow instead of forcing refresh
      const result = await authWithBlackboxDeviceFlow(client, config);
      if (!result.success) {
        throw new Error('Blackbox OAuth authentication failed');
      }
      return client;
    }
    ...
    if (!result.success) {
      ...
      switch (result.reason) {
        case 'timeout':
          throw new Error('Blackbox OAuth authentication timed out');
        case 'cancelled':
          throw new Error('Blackbox OAuth authentication was cancelled by user');
        case 'rate_limit':
          throw new Error(
...
async function authWithBlackboxDeviceFlow(
  client: BlackboxOAuth2Client,
  config: Config,
): Promise<AuthResult> {
  ...
  try {
    ...
    const showFallbackMessage = () => {
      console.log('\n=== Blackbox OAuth Device Authorization ===');
      console.log(
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        ...
        if (isDeviceTokenSuccess(tokenResponse)) {
          ...
          // Convert to BlackboxCredentials format
          ...
          await cacheBlackboxCredentials(credentials);
...
async function loadCachedBlackboxCredentials(
  client: BlackboxOAuth2Client,
): Promise<boolean> {
  try {
    const keyFile = getBlackboxCachedCredentialPath();
...
async function cacheBlackboxCredentials(credentials: BlackboxCredentials) {
  const filePath = getBlackboxCachedCredentialPath();
...
/**
 * Clear cached Blackbox credentials from disk
 * This is useful when credentials have expired or need to be reset
 */
export async function clearBlackboxCredentials(): Promise<void> {
  try {
    const filePath = getBlackboxCachedCredentialPath();
    await fs.unlink(filePath);
    console.debug('Cached Blackbox credentials cleared successfully.');
  } catch (error: unknown) {
    ...
    console.warn('Warning: Failed to clear cached Blackbox credentials:', error);
  }
}
