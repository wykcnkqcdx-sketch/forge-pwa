...
const BLACKBOX_LOCK_FILENAME = 'oauth_creds.lock';
...
interface MemoryCache {
  credentials: BlackboxCredentials | null;
  fileModTime: number;
  lastCheck: number;
}

/**
 * Validates that the given data is a valid BlackboxCredentials object
 *
 * @param data - The data to validate
 * @returns The validated credentials object
 * @throws Error if the data is invalid
 */
function validateCredentials(data: unknown): BlackboxCredentials {
  if (!data || typeof data !== 'object') {
    throw new Error('Invalid credentials format');
  }

  const creds = data as Partial<BlackboxCredentials>;
  ...
  return creds as BlackboxCredentials;
}
...
export class SharedTokenManager {
   ...
   */
  private refreshPromise: Promise<BlackboxCredentials> | null = null;
   ...
   *
   * @param blackboxClient - The OAuth2 client instance
  ...
  async getValidCredentials(
    ...
    try {
      ...
      await this.checkAndReloadIfNeeded(blackboxClient);
  ...
  private async checkAndReloadIfNeeded(
    ...
    this.checkPromise = this.performFileCheck(blackboxClient, now);
  ...
  private async performFileCheck(
    blackboxClient: IBlackboxOAuth2Client | undefined,
    checkTime: number,
  ): Promise<void> {
    ...
    try {
      ...
      if (fileModTime > this.memoryCache.fileModTime) {
        await this.reloadCredentialsFromFile(blackboxClient);
   ...
   */
  private async forceFileCheck(blackboxClient?: IBlackboxOAuth2Client): Promise<void> {
    try {
      const filePath = this.getCredentialFilePath();
  ...
  /**
   * Load credentials from the file system into memory cache and sync with blackboxClient
   */
  private async reloadCredentialsFromFile(
    ...
    try {
      ...
      // Sync with blackboxClient atomically - rollback on failure
      try {
        if (blackboxClient) {
          blackboxClient.setCredentials(credentials);
        }
      } catch (clientError) {
  ...
  private async performTokenRefresh(
    ...
    try {
      ...
      const currentCredentials = blackboxClient.getCredentials();
      ...
      await this.forceFileCheck(blackboxClient);
      ...
      if (
        ...
        // No need to call blackboxClient.setCredentials here as checkAndReloadIfNeeded already did it
        return this.memoryCache.credentials;
      }

      // Perform the actual token refresh
      const response = await blackboxClient.refreshAccessToken();
   ...
   */
  private isTokenValid(credentials: BlackboxCredentials): boolean {
  ...
  private getLockFilePath(): string {
    return path.join(os.homedir(), BLACKBOX_DIR, BLACKBOX_LOCK_FILENAME);
  }
