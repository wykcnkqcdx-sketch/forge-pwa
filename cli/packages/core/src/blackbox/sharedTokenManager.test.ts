 ...
 */
function createMockBlackboxClient(
  initialCredentials: Partial<BlackboxCredentials> = {},
): IBlackboxOAuth2Client {
  let credentials: BlackboxCredentials = {
  ...
  return {
    setCredentials: vi.fn((creds: BlackboxCredentials) => {
...
function createValidCredentials(
  overrides: Partial<BlackboxCredentials> = {},
): BlackboxCredentials {
  return {
  ...
  describe('CredentialsClearRequiredError handling', () => {
    it('should clear memory cache when CredentialsClearRequiredError is thrown during refresh', async () => {
      ...
      const refreshPromise =
        getPrivateProperty<Promise<BlackboxCredentials> | null>(
          tokenManager,
          'refreshPromise',
        );
    ...
    it('should properly clean up timeout when file operation completes before timeout', async () => {
      ...
      ) as (client?: IBlackboxOAuth2Client) => Promise<void>;
