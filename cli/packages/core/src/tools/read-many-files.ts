...
export interface ReadManyFilesParams {
  ...
  /**
   * Whether to respect .gitignore and .blackboxignore patterns (optional, defaults to true)
   */
...
${finalExclusionPatternsForDescription
  ...
  .join(
    ...
    // Add a note if .blackboxignore patterns contributed to the final list of exclusions
    if (geminiIgnorePatterns.length > 0) {
      const geminiPatternsInEffect = geminiIgnorePatterns.filter((p) =>
        finalExclusionPatternsForDescription.includes(p),
      ).length;
      if (geminiPatternsInEffect > 0) {
        excludeDesc += ` (includes ${geminiPatternsInEffect} from .blackboxignore)`;
      }
    }
  ...
  constructor(private config: Config) {
    const parameterSchema = {
      ...
      properties: {
        ...
        file_filtering_options: {
          description:
            'Whether to respect ignore patterns from .gitignore or .blackboxignore',
          type: 'object',
          properties: {
            respect_git_ignore: {
              description:
            ...
            respect_gemini_ignore: {
              description:
                'Optional: Whether to respect .blackboxignore patterns when listing files. Defaults to true.',
              type: 'boolean',
            },
          },
        },
