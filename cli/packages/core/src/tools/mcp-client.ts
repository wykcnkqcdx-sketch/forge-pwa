  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      this.client.onerror = (error) => {
        ...
        // Only log errors in debug mode - errors are stored and displayed in UI
        if (this.debugMode) {
          console.error(`[DEBUG] MCP ERROR (${this.serverName}):`, error.toString());
        }
...
export async function connectAndDiscover(
  ...
  try {
    ...
    mcpClient.onerror = (error) => {
      // Only log errors in debug mode - errors are stored and displayed in UI
      if (debugMode) {
        console.error(`[DEBUG] MCP ERROR (${mcpServerName}):`, error.toString());
      }
