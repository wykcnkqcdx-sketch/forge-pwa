...
interface BlackboxAIAnnotation {
  type: string;
  url_citation?: {
    title?: string;
    url?: string;
...
> {
  ...
  private isBlackboxAIProvider(): boolean {
    const contentGenConfig = this.config.getContentGeneratorConfig();
    return (
      contentGenConfig?.authType === AuthType.USE_BLACKBOX_API
    );
  }

  private async executeBlackboxAISearch(
    signal: AbortSignal,
  ): Promise<WebSearchToolResult> {
    try {
      const contentGenConfig = this.config.getContentGeneratorConfig();
      const baseURL = contentGenConfig?.baseUrl;
      const apiKey = contentGenConfig?.apiKey;

      const searchModel = 'blackboxai/blackbox-search';
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `BLACKBOX AI API error: ${response.status} ${response.statusText}${text ? ` - ${text}` : ''}`,
        );
      }
      ...
      const sources = annotations
        .filter((a: BlackboxAIAnnotation) => a.type === 'url_citation')
        .map((a: BlackboxAIAnnotation) => ({
          title: a.url_citation?.title || 'Untitled',
      ...
      if (sources.length > 0) {
        const sourceList = sources
          .map((s: { title: string; url: string }, i: number) => `[${i + 1}] ${s.title} (${s.url})`)
          .join('\n');
        formattedContent += `\n\nSources:\n${sourceList}`;
      }
      ...
      const errorMessage = `Error during BLACKBOX AI web search for query "${this.params.query}": ${getErrorMessage(
        error,
      )}`;
  ...
  async execute(signal: AbortSignal): Promise<WebSearchToolResult> {
    // Check if using BLACKBOX AI provider
    if (this.isBlackboxAIProvider()) {
      return this.executeBlackboxAISearch(signal);
    }

    // Fall back to Tavily search
    return this.executeTavilySearch(signal);
  }
}
