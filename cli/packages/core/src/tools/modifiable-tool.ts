...
function createTempFilesForModify(
  ...
  const diffDir = path.join(tempDir, 'blackbox-cli-tool-modify-diffs');
  ...
  const tempOldPath = path.join(
    diffDir,
    `blackbox-cli-modify-${fileName}-old-${timestamp}${ext}`,
  );
  const tempNewPath = path.join(
    diffDir,
    `blackbox-cli-modify-${fileName}-new-${timestamp}${ext}`,
  );
