...
> {
  ...
  private async executeDirectFetch(signal: AbortSignal): Promise<ToolResult> {
    ...
    try {
      console.debug(
        `[WebFetchTool] Fetching content from: ${url} (with cache-busting enabled)`,
      );
      ...
      const fallbackPrompt = `Current Date and Time: ${currentDateTime}

The user requested the following: "${this.params.prompt}".

I have fetched the LATEST content from ${this.params.url} (fetched on ${currentDate} with cache-busting to ensure freshness). Please use the following content to answer the user's request.

IMPORTANT INSTRUCTIONS FOR HANDLING RECENT DATA:
1. When the user asks for "latest" information, actively search through the content for the most recent dates, versions, or releases mentioned.
2. If you don't have knowledge about recent developments in your training data, rely ENTIRELY on the fetched content below - it contains the most up-to-date information available.
3. Prioritize information with recent dates (close to ${currentDate}) over older information. Always mention the dates/versions when presenting the data to show recency.
