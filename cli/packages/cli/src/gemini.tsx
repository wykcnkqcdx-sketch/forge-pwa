...
import { validateAuthMethod, loadProviderCredentialsFromSettings } from './config/auth.js';
...
export async function main() {
  ...
  // Load provider credentials from settings and set as environment variables
  ...
  // Handle list checkpoints command after config is initialized
  if (config.getListCheckpoints()) {
    const { listCheckpoints } = await import('./nonInteractiveCli.js');
  ...
  // Add MCP server connection errors to startup warnings
  if (config.hasMcpServerErrors()) {
    const mcpErrors = config.getMcpServerErrors();
    for (const mcpError of mcpErrors) {
      let warningMessage = `⚠️  MCP Server "${mcpError.serverName}" failed to connect:\n   ${mcpError.userFriendlyMessage}`;
      if (config.getDebugMode()) {
        warningMessage += `\n   Technical details: ${mcpError.error.message}`;
      }
      startupWarnings.push(warningMessage);
    }
  }
...
function setWindowTitle(title: string, settings: LoadedSettings) {
  if (!settings.merged.ui?.hideWindowTitle) {
    const windowTitle = (process.env['CLI_TITLE'] || `Blackbox - ${title}`).replace(
