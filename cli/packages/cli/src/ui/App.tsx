...
import type { EditorType, Config, IdeContext } from '@blackbox_ai/blackbox-cli-core';
import {
  ApprovalMode,
...
const App = ({ config, settings, startupWarnings = [], version }: AppProps) => {
  ...
  useBracketedPaste();
  const [updateInfo, _setUpdateInfo] = useState<UpdateObject | null>(null);
  ...
  useEffect(() => {
    if (isBlackboxAuth && authStatus === 'timeout') {
      setAuthError(
        authMessage ||
          'Blackbox OAuth authentication timed out. Please try again or select a different authentication method.',
      );
      cancelBlackboxAuth();
      cancelAuthentication();
      openAuthDialog();
    ...
    addItem(
      {
        ...
        text: 'Refreshing hierarchical memory (BLACKBOX.md or other context files)...',
      },
      Date.now(),
    );
    try {
...
⚡ You can switch authentication methods by typing /auth`;
  ...
  const [availableModelsForDialog, setAvailableModelsForDialog] = useState<AvailableModel[]>([]);
      ...
      <Box flexDirection="column" width="90%">
        ...
        <Box flexDirection="column" ref={mainControlsRef}>
          ...
          {showWorkspaceMigrationDialog ? (
                ...
                onResume={async (tag) => {
                  try {
                    ...
                    if (conversation) {
                      ...
                      const historyWithIds: HistoryItem[] = uiHistory.map((item, index) => ({
                        ...item,
                        id: index,
                      }));
            ...
            <>
              {isBlackboxAuth && isBlackboxAuthenticating ? (
                <BlackboxOAuthProgress
                  deviceAuth={deviceAuth || undefined}
                  authStatus={authStatus}
                  authMessage={authMessage}
                  onTimeout={() => {
                    setAuthError(
                      'Blackbox OAuth authentication timed out. Please try again.',
                    );
                    cancelBlackboxAuth();
                    cancelAuthentication();
                    openAuthDialog();
                  ...
                  onCancel={() => {
                    setAuthError('Blackbox OAuth authentication cancelled.');
                    cancelBlackboxAuth();
                    cancelAuthentication();
                    openAuthDialog();
                  }}
