...
import { AuthType, type Config, ApprovalMode } from '@blackbox_ai/blackbox-cli-core';
...
describe('useVisionAutoSwitch helpers', () => {
  describe('shouldOfferVisionSwitch', () => {
    it('returns false when authType is not BLACKBOX_OAUTH', () => {
    ...
    it('returns true when image parts exist, BLACKBOX_OAUTH, and model is not vision', () => {
      const parts: PartListUnion = [
        { text: 'hello' },
    ...
    it('returns false when authType is not BLACKBOX_OAUTH in YOLO mode context', () => {
      const parts: PartListUnion = [
        { inlineData: { mimeType: 'image/png', data: '...' } },
      ];
      const result = shouldOfferVisionSwitch(
...
describe('useVisionAutoSwitch hook', () => {
  ...
  it('returns shouldProceed=true immediately for continuations', async () => {
    const config = createMockConfig(AuthType.BLACKBOX_OAUTH, 'blackbox3-coder-plus');
  ...
  it('does nothing when authType is not BLACKBOX_OAUTH', async () => {
    const config = createMockConfig(AuthType.USE_GEMINI, 'blackbox3-coder-plus');
  ...
  it('applies a one-time override and returns originalModel, then restores', async () => {
    const initialModel = 'blackbox3-coder-plus';
    const config = createMockConfig(AuthType.BLACKBOX_OAUTH, initialModel);
  ...
  describe('YOLO mode behavior', () => {
    ...
    it('does not switch in YOLO mode when authType is not BLACKBOX_OAUTH', async () => {
      const config = createMockConfig(
        AuthType.USE_GEMINI,
        'blackbox3-coder-plus',
        ApprovalMode.YOLO,
  ...
  describe('VLM switch mode default behavior', () => {
    it('should automatically switch once when vlmSwitchMode is "once"', async () => {
      ...
      expect(switchResult.originalModel).toBe('blackbox3-coder-plus');
