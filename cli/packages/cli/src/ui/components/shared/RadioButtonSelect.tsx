...
export function RadioButtonSelect<T>({
  ...
  // Only reset activeIndex when items list changes significantly (length changes)
  // This prevents flickering when typing fast in search
  useEffect(() => {
    ...
    if (itemsLengthChanged) {
      // Clamp activeIndex to valid range when items length changes
      if (items.length === 0) {
        setActiveIndex(0);
        setScrollOffset(0);
      } else {
        const newIndex = Math.min(Math.max(0, activeIndex), items.length - 1);
        setActiveIndex(newIndex);
        // Reset scroll offset when items change significantly
        setScrollOffset(0);
      }
    } else if (itemsChanged && items.length > 0) {
      // Items changed but length is the same - just ensure activeIndex is valid
      const newIndex = Math.min(activeIndex, items.length - 1);
  ...
  // Handle scroll offset updates separately to avoid cascading updates
  useEffect(() => {
  ...
  // Memoize the keypress handler to prevent unnecessary re-renders
  const handleKeypress = useCallback(
    (key: { sequence: string; name: string }) => {
    ...
    [activeIndex, items, numberInput, onHighlight, onSelect, showNumbers],
  );

  useKeypress(handleKeypress, { isActive: !!(isFocused && items.length > 0) });

  // Memoize visible items calculation to prevent unnecessary recalculations
  const visibleItems = useMemo(
    () => items.slice(scrollOffset, scrollOffset + maxItemsToShow),
    [items, scrollOffset, maxItemsToShow],
  );

  // Memoize number column width calculation
  const numberColumnWidth = useMemo(
  ...
  return (
    <Box flexDirection="column">
      ...
      {visibleItems.map((item, index) => {
        ...
        return (
          <Box key={`${scrollOffset}-${index}`} alignItems="center">
