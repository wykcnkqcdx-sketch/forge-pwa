...
import { ToolConfirmationOutcome, ApprovalMode } from '@blackbox_ai/blackbox-cli-core';
...
> = ({
  ...
  const handleConfirm = async (outcome: ToolConfirmationOutcome) => {
    // Handle YOLO mode enablement
    if (outcome === ToolConfirmationOutcome.EnableYoloMode) {
      try {
        config.setApprovalMode(ApprovalMode.YOLO);
        // After enabling YOLO mode, proceed with the current tool
        onConfirm(ToolConfirmationOutcome.ProceedOnce);
        return;
      } catch (_error) {
        // If enabling YOLO mode fails, just cancel
  ...
  if (confirmationDetails.type === 'edit') {
    ...
    if (isTrustedFolder) {
      options.push({
        label: 'Never ask for any tools (YOLO mode)',
        value: ToolConfirmationOutcome.EnableYoloMode,
      });
    }
