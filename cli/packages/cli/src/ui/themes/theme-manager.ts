...
import { BlackboxLight } from './blackbox-light.js';
import { BlackboxDark } from './blackbox-dark.js';
...
export const DEFAULT_THEME: Theme = BlackboxDark;
...
class ThemeManager {
  ...
  getAvailableThemes(): ThemeDisplay[] {
    ...
    // Separate Blackbox themes
    const blackboxCliThemes = builtInThemes.filter(
      (theme) => theme.name === BlackboxLight.name || theme.name === BlackboxDark.name,
    );
    ...
    // Combine Blackbox themes first, then sorted others
    return [...blackboxCliThemes, ...sortedOtherThemes];
  }
