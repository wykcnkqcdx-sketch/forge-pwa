...
import { decodeTagName } from '@blackbox_ai/blackbox-cli-core';
...
const resumeCommand: SlashCommand = {
  ...
  completion: async (context, partialArg) => (await getSavedChatTags(context, true))
    .map((chat) => chat.name)
...
const browseCommand: SlashCommand = {
  name: 'history',
  altNames: ['browse'],
  description: 'Browse saved conversations interactively',
  kind: CommandKind.BUILT_IN,
  action: async (): Promise<OpenDialogActionReturn> => ({
    type: 'dialog',
    dialog: 'history_browser',
  }),
};
