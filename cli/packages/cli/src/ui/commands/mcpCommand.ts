...
import type { DiscoveredMCPPrompt } from '@blackbox_ai/blackbox-cli-core';
import {
  DiscoveredMCPTool,
...
const getMcpStatus = async (
  ...
  if (serverNames.length === 0 && blockedMcpServers.length === 0) {
    const docsUrl =
      'https://blackbox_ai.github.io/blackbox-cli-docs/en/tools/mcp-server/#how-to-set-up-your-mcp-server';
    return {
      type: 'message',
      messageType: 'info',
