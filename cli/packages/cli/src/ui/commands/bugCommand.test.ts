...
describe('bugCommand', () => {
  beforeEach(() => {
    vi.mocked(getCliVersion).mockResolvedValue('0.1.2');
    vi.mocked(formatMemoryUsage).mockReturnValue('100 MB');
    vi.mock('@blackbox_ai/blackbox-cli-core', () => ({
      sessionId: 'test-session-id',
    }));
    vi.stubEnv('SANDBOX', 'blackbox-test');
  });
...
* **Model Version:** blackbox3-coder-plus
* **Memory Usage:** 100 MB
* **IDE Client:** VSCode
`;
    const expectedUrl =
      'https://github.com/llmcod/blackbox_cli/issues/new?template=bug_report.yml&title=A%20test%20bug&info=' +
