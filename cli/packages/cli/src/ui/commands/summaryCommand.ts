...
import { getProjectSummaryPrompt } from '@blackbox_ai/blackbox-cli-core';
import type { HistoryItemSummary } from '../types.js';

export const summaryCommand: SlashCommand = {
  name: 'summary',
  description:
    'Generate a project summary and save it to .blackboxcli/PROJECT_SUMMARY.md',
  ...
  action: async (context): Promise<SlashCommandActionReturn> => {
    ...
    try {
      ...
      // Ensure .blackboxcli directory exists
      const projectRoot = config.getProjectRoot();
      const blackboxDir = path.join(projectRoot, '.blackboxcli');
      try {
        await fsPromises.mkdir(blackboxDir, { recursive: true });
      } catch (_err) {
        // Directory might already exist, ignore error
      }

      // Save the summary to PROJECT_SUMMARY.md
      const summaryPath = path.join(blackboxDir, 'PROJECT_SUMMARY.md');
...
`;
      ...
      const completedSummaryItem: HistoryItemSummary = {
        ...
        summary: {
          isPending: false,
          stage: 'completed',
          filePath: '.blackboxcli/PROJECT_SUMMARY.md',
        },
      };
