...
import { Storage } from '@blackbox_ai/blackbox-cli-core';
...
export const EXTENSIONS_DIRECTORY_NAME = path.join('.blackboxcli', 'extensions');
export const EXTENSIONS_CONFIG_FILENAME = 'blackbox-extension.json';
export const EXTENSIONS_CONFIG_FILENAME_OLD = 'gemini-extension.json';
export const INSTALL_METADATA_FILENAME = '.blackboxcli-extension-install.json';
...
export class ExtensionStorage {
  ...
  static async createTmpDir(): Promise<string> {
    return await fs.promises.mkdtemp(path.join(os.tmpdir(), 'blackbox-extension'));
  }
}
...
export async function installExtension(
  ...
  try {
    ...
    if (!newExtension) {
      throw new Error(
        `Invalid extension at ${installMetadata.source}. Please make sure it has a valid blackbox-extension.json file.`,
      );
    }

    // ~/.blackboxcli/extensions/{ExtensionConfig.name}.
...
export async function updateExtension(
  ...
  if (!extension.installMetadata) {
    throw new Error(
      `Extension cannot be updated because it is missing the .blackboxcli-extension.install.json file. To update manually, uninstall and then reinstall the updated version.`,
    );
  }
