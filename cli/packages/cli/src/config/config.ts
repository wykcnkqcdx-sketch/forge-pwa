...
export interface CliArgs {
  ...
  saveCheckpoint: string | undefined;
  autoSave: boolean | undefined;
  resumeCheckpoint: string | undefined;
  listCheckpoints: boolean | undefined;
}
...
export async function parseArguments(settings: Settings): Promise<CliArgs> {
    ...
    .command('$0', 'Launch Blackbox Code', (yargsInstance) =>
      yargsInstance
        ...
        .option('proxy', {
          type: 'string',
          description:
            'Proxy for blackbox client, like schema://user:password@host:port',
        })
        ...
        .option('supress-tool-errors', {
          type: 'boolean',
          alias: 'ste',
          description: 'Supress tool errors.',
          default: true,
        })
        ...
        .option('save-checkpoint', {
          type: 'string',
          description: 'Save conversation as checkpoint with the given tag after task completion (non-interactive mode only)',
        })
        .option('auto-save', {
          type: 'boolean',
          description: 'Automatically save conversation checkpoint after task completion (non-interactive mode only)',
          default: false,
        })
        .option('resume-checkpoint', {
          type: 'string',
          description: 'Resume from a saved checkpoint with the given tag (non-interactive mode only)',
        })
        .option('list-checkpoints', {
          type: 'boolean',
        ...
        .check((argv) => {
          ...
          if (argv['saveCheckpoint'] && argv['autoSave']) {
            throw new Error(
              'Cannot use both --save-checkpoint and --auto-save together. Choose one.',
            );
          }
          if ((argv['saveCheckpoint'] || argv['autoSave']) && argv['promptInteractive']) {
            throw new Error(
              'Checkpoint saving flags (--save-checkpoint, --auto-save) are only supported in non-interactive mode. Remove --prompt-interactive (-i) to use these flags.',
            );
          }
          if ((argv['saveCheckpoint'] || argv['autoSave']) && !argv.prompt && process.stdin.isTTY) {
            throw new Error(
              'Checkpoint saving flags (--save-checkpoint, --auto-save) require non-interactive mode. Use --prompt (-p) or pipe input to stdin.',
            );
          }
          if (argv['resumeCheckpoint'] && argv['promptInteractive']) {
            throw new Error(
              'Cannot use --resume-checkpoint with --prompt-interactive (-i). Resume checkpoint is only supported in non-interactive mode.',
            );
          }
          if (argv['resumeCheckpoint'] && argv['saveCheckpoint']) {
            throw new Error(
              'Cannot use --resume-checkpoint with --save-checkpoint. Use --auto-save instead if you want to continue saving checkpoints after resuming.',
            );
          }
          if (argv['listCheckpoints'] && (argv.prompt || argv['promptInteractive'])) {
            throw new Error(
              'Cannot use --list-checkpoints with prompt flags. Use --list-checkpoints alone to list checkpoints.',
            );
          }
          return true;
        }),
  ...
  // Handle case where MCP/configure/update subcommands are executed - they should exit the process
  // and not return to main CLI logic
  if (
    result._.length > 0 &&
    (result._[0] === 'mcp' || result._[0] === 'extensions' || result._[0] === 'configure' || result._[0] === 'update')
  ) {
...
export async function loadCliConfig(
  ...
  return new Config({
    ...
    vlmSwitchMode,
    saveCheckpoint: argv.saveCheckpoint,
    autoSave: argv.autoSave,
    resumeCheckpoint: argv.resumeCheckpoint,
    listCheckpoints: argv.listCheckpoints,
  });
}
