# Sandboxing in Blackbox Code

This document provides a guide to sandboxing in Blackbox Code, including prerequisites, quickstart, and configuration.

## Prerequisites

Before using sandboxing, you need to install and set up Blackbox Code:
...
blackbox -s -p "analyze the code structure"

# Use environment variable
export GEMINI_SANDBOX=true
blackbox -p "run the test suite"
...
```bash
DEBUG=1 blackbox -s -p "debug command"
```

**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect the CLI due to automatic exclusion. Use `.blackboxcli/.env` files for Blackbox Code-specific debug settings.

### Inspect sandbox

```bash
# Check environment
blackbox -s -p "run shell command: env | grep SANDBOX"

# List mounts
blackbox -s -p "run shell command: mount | grep workspace"
```
