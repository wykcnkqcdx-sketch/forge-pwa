...
console.log('packing @blackbox_ai/blackbox-cli ...');
const cliPackageDir = join('packages', 'cli');
rmSync(join(cliPackageDir, 'dist', 'blackbox-cli-*.tgz'), { force: true });
execSync(
  `npm pack -w @blackbox_ai/blackbox-cli --pack-destination ./packages/cli/dist`,
  {
    stdio: 'ignore',
  },
);

console.log('packing @blackbox_ai/blackbox-cli-core ...');
const corePackageDir = join('packages', 'core');
rmSync(join(corePackageDir, 'dist', 'blackbox-cli-core-*.tgz'), {
  force: true,
});
execSync(
  `npm pack -w @blackbox_ai/blackbox-cli-core --pack-destination ./packages/core/dist`,
  { stdio: 'ignore' },
);
...
function buildImage(imageName, dockerfile) {
  ...
  if (sandboxCommand === 'podman') {
    if (isWindows) {
      ...
      tempAuthFile = join(os.tmpdir(), `blackbox-auth-${Date.now()}.json`);
