...
const USER_GEMINI_DIR = path.join(os.homedir(), '.blackboxcli');
// Project-level .gemini directory in the workspace
const WORKSPACE_GEMINI_DIR = path.join(projectRoot, '.blackboxcli');

// Telemetry artifacts are stored in a hashed directory under the user's ~/.blackboxcli/tmp
...
export function getJson(url) {
  const tmpFile = path.join(
    os.tmpdir(),
    `blackbox-cli-releases-${Date.now()}.json`,
  );
  try {
    execSync(
      `curl -sL -H "User-Agent: blackbox-cli-dev-script" -o "${tmpFile}" "${url}"`,
      { stdio: 'pipe' },
    );
...
export async function ensureBinary(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'blackbox-cli-telemetry-'));
