...
- Added Blackbox3-VL-Plus token limits (256K input, 32K output) and highres support
...
- Fixed authentication hang when selecting Blackbox OAuth.
- Added OpenAI and Blackbox OAuth authentication support to Zed ACP integration.
- Fixed output token limit for Blackbox models.
...
- Added vision model support for Blackbox-OAuth authentication.
...
- Re-implemented tokenLimits class for better compatibility with Blackbox and other model types.
...
- Fixed token synchronization among multiple Blackbox sessions.
...
- Renamed `GEMINI_DIR` to `BLACKBOX_DIR` for better branding consistency.
- Added support for Blackbox Markdown selection.
...
- `grep` now respects `.blackboxignore`.
- Miscellaneous improvements and bug fixes.
...
- Corrected Blackbox logger exit-handler setup.
- Separated static QR code and dynamic spinner components.

## 0.0.6

- Added usage statistics logging for Blackbox integration.
- Made `/init` respect the configured context filename and aligned docs with `BLACKBOX.md`.
