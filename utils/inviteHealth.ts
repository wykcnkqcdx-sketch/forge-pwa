...
export function displayInviteStatus(invite: CloudInvite, now: Date): CloudInvite['status'] {
  if (invite.status === 'pending' && new Date(invite.expiresAt).getTime() <= now.getTime()) return 'expired';
  return invite.status;
}

export function isStalePendingInvite(invite: CloudInvite, now = new Date()) {
  return displayInviteStatus(invite, now) === 'pending'
...
export function buildInviteHealth(members: SquadMember[], cloudInvites: CloudInvite[], now = new Date()) {
  ...
  const stalePending = cloudInvites.filter((invite) => isStalePendingInvite(invite, now)).length;
